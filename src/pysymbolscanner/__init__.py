...
def get_missing_objects(index, scanner):
    missing_stocks = list(
        filter(
            lambda x: x['name']
            not in map(
...
def get_wrong_objects(index, scanner):
    wrong_stocks = list(
        filter(
            lambda x: x['name']
...
if __name__ == '__main__':
    ...
    with open(args.input, 'r') as in_file:
        ...
        for index in indices:
            missing = get_missing_objects(index, scanner)
            wrong = get_wrong_objects(index, scanner)
